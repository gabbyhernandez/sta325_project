...
gam1 <- gam(Life.expectancy ~ s(Polio) + s(Diphtheria) + s(GDP) +s(Population) + s(BMI) +s(Adult.Mortality_Status) + s(under.five.deaths_Status) + s(GDP_Status) + s(Population_Status), data = imputed_data)
...
# Make predictions on the test set
predictions <- predict(gam1, newdata = imputed_data, type = "response")

# Calculate Mean Squared Error (MSE)
mse <- mean((predictions - imputed_data$Life.expectancy)^2)
cat("Mean Squared Error (MSE):", mse, "\n")

# Calculate R-squared (R^2)
ss_residual <- sum((imputed_data$Life.expectancy - predictions)^2)
ss_total <- sum((imputed_data$Life.expectancy - mean(imputed_data$Life.expectancy))^2)
r_squared <- 1 - (ss_residual / ss_total)
cat("R-squared (R^2):", r_squared, "\n")
```
...
gam.ns <- lm(Life.expectancy ~ ns(Polio) + ns(Diphtheria) + ns(GDP) + ns(Population) + ns(BMI) + ns(Adult.Mortality_Status) + ns(under.five.deaths_Status) + ns(GDP_Status) + ns(Population_Status), data = imputed_data)
plot.Gam(gam.ns, se = TRUE, col = "blue")
summary(gam.ns)

# Make predictions on the test set
predictions <- predict(gam.ns, newdata = imputed_data, type = "response")
...
gam2<- gam(Life.expectancy ~ s(Polio) + s(Diphtheria) + s(Adult.Mortality) + s(under.five.deaths) + s(HIV.AIDS) + s(GDP) +s(Population) + s(BMI), data = imputed_data)
...
# Make predictions on the test set
predictions <- predict(gam2, newdata = imputed_data, type = "response")
...
gam.m1 <- gam(Life.expectancy ~ s(Polio) + s(Diphtheria) + s(Adult.Mortality) + s(under.five.deaths) + s(GDP) + s(BMI), data = imputed_data)
gam.m2 <- gam(Life.expectancy ~ s(Polio) + s(Diphtheria) + s(Adult.Mortality) + s(under.five.deaths) + s(HIV.AIDS) + GDP + Population + s(BMI), data = imputed_data)
plot(gam.m2)

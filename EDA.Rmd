...
Looking for general trends between immunizations and life expectancy:
```{r}
data$Measles <- pmin(data$Measles, 1000)
data$Polio <- pmin(data$Polio, 1000)

plot(Life.expectancy~Hepatitis.B,data=data, main = "Hepatitis B Immunization Among 1-Year-Olds (%) vs. Life Expectancy")
plot(Life.expectancy~Measles,data=data, fill = Status, main = "# Reported Measles cases (per 1000) vs. Life Expectancy")
plot(Life.expectancy~Polio,data=data, main = "# Reported Polio cases (per 1000) vs. Life Expectancy")
plot(Life.expectancy~Diphtheria,data=data, main = "DTP3 Immunization Among 1-Year-Olds (%) vs. Life Expectancy")
...
```{r}
library(grplasso)
X <- imputed_data[, c("Hepatitis.B", "Polio", "Diphtheria", "Adult.Mortality", "under.five.deaths", "HIV.AIDS", "percentage.expenditure", "GDP", "Income.composition.of.resources", "Alcohol", "Population", "BMI", "Schooling")]
X <- as.matrix(X)
y <- imputed_data$Life.expectancy
group_indices <- list(c(1, 2, 3), c(4, 5, 6), c(7, 8, 9), c(10, 11, 12,13))
fit <- grplasso(X, y = y, group = group_indices, model = LogReg(), lambda = 20,
                center = TRUE, standardize = TRUE)
coef(fit)
cvfit <- cv.grplasso(X, y = y, group = group_indices, model = LogReg())
cvfit$lambda.min
plot(cvfit)
```
